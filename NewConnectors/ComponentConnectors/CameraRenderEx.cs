...
namespace Thundagun.NewConnectors.ComponentConnectors;
  ...
  public class CameraRenderEx : MonoBehaviour
  {
    ...
    private void OnPreRender()
    {
      try
      {
        var num1 = MathX.Clamp(MathX.FilterInvalid(!UseTransformScale ? 1f : MathX.AvgComponent(Camera.transform.lossyScale.ToEngine())), 1E-05f, 1000000f);
        var num2 = MathX.Clamp(OrthographicSize * num1, 1E-06f, 1000000f);
        var num3 = MathX.Clamp(NearClip * num1, 0.0001f, 1000000f);
        var num4 = MathX.Clamp(FarClip * num1, MathX.Max(0.0001f, num3 + 0.0001f), 1000000f);
        Camera.orthographicSize = num2;
        Camera.nearClipPlane = num3;
        Camera.farClipPlane = num4;
        if (Texture != null)
        ...
        if (Camera.rect != new UnityEngine.Rect(0.0f, 0.0f, 1f, 1f))
        {
          ...
          descriptor.height = (int) (rect.height * (double) descriptor.height);
          descriptor.width = (int) (rect.width * (double) descriptor.width);
          _prevRect = rect;
    ...
    private void OnPostRender()
    {
      try
      {
        ...
        {
          Graphics.CopyTexture(Camera.targetTexture, 0, 0, 0, 0, Camera.targetTexture.width,
            Camera.targetTexture.height, _prevTexture, 0, 0, (int) (_prevRect.Value.x * (double) _prevTexture.width),
            (int) (_prevRect.Value.y * (double) _prevTexture.height));
